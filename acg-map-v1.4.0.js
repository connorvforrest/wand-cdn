/*
  acg-map v1.4.0 (CDN file acg-map-v1.4.0.js; v1.3.0 stays at acg-map-v1.3.0.js, v1.2.0 at
  acg-map-v1.2.0.js, v1.1.0 at acg-map-v1.1.0.js, v1.0.0 at acg-map-v1.js). The
  ...
  Loader line: <script src="https://cdn.allcare-ga.com/acg-map-v1.4.0.js"
  ...
  Popup (v1.2.0, Connor's second staging review, item 8): Book is the primary button and
  comes first, Directions is a link button, and there is no Call button. v1.3.0 (Connor's
  third review): the clinic name goes into the info window's own header row, beside the
  close button, through InfoWindow.setHeaderContent. Google reserves that row whether or
  not it holds anything, so an empty one was a band of white above every popup. A Maps API
  without setHeaderContent keeps the name inside the content, as before.

  Card taps (v1.4.0, Connor's fourth review, STUD-276 item 5): at 767px and down the map
  sits under the list, so a card tap no longer scrolls the page down to it; the pin still
  opens and the map pans for when the visitor gets there. Beside the list, on wider
  screens, the map scrolls into view as before.
...
(function () {
  ...
  var CSS = '[data-acg-map]{position:relative;width:100%;height:100%;min-height:480px;border-radius:12px;background:#eef2f6;overflow:hidden}'
    ...
    + '.acg-pop-name{color:#111827;font-size:15px;font-weight:700}.acg-pop .acg-pop-name{margin-bottom:4px}'
    ...
    + '.acg-pop-btn.is-link{flex:0 0 auto;background:none;color:#1D4987;text-decoration:underline}'
  ...
  // The name as an element for the info window's header row (v1.3.0).
  function infoHeader(loc) {
    var el = document.createElement('div');
    el.className = 'acg-pop-name';
    el.textContent = loc.display;
    return el;
  }

  function infoHtml(slug, loc, state, nameInBody) {
    ...
    return [
      '<div class="acg-pop">',
      nameInBody ? '<div class="acg-pop-name">' + loc.display + '</div>' : '',
      ...
      // Book first and primary, Directions a link, no Call (v1.2.0). data-clinic="book-href" +
      // data-location: the library's observer keeps the href current (visit_type, modality,
      // the ad tags) and its click event reports the tap (v1.1.0).
      ...
      '<a class="acg-pop-btn is-link" href="', dir, '" target="_blank" rel="noopener"',
  ...
  function openInfo(slug) {
    if (!info) return;
    var hasHeader = typeof info.setHeaderContent === 'function';
    if (hasHeader) info.setHeaderContent(infoHeader(clinics[slug]));
    info.setContent(infoHtml(slug, clinics[slug], window.ACG.getLocation(), !hasHeader));
  ...
  // Is the map beside the list, or under it? At 767px and down every page stacks the map
  // under the cards (the /locations column order, the Map Section's stack), and a card
  // tap must not carry the visitor down to it (v1.4.0).
  function mapBesideList() {
  ...
  // the card still navigate. The page scrolls to the map only where the map is beside the
  // list.
  function onCardClick(e) {
    if (e.target.closest && e.target.closest('a, button')) return;
    var slug = slugFromEvent(e);
    if (!slug || !markers[slug] || !map) return;
    if (mapBesideList() && mapEl.scrollIntoView) mapEl.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
