/**
 * ACG Vox booking split — v1.1.0 (2026-08-11): provider + service handoff.
 *   - buildHandoff forwards `service` + `provider` (lowercased, _→-), per Vox 8/8 confirm
 *     ("provider=provider-slug") + their form's careType = service ?? visit_type.
 *   - The blanket ?w= skip narrows to ?w=-WITHOUT-provider: lib v1.19 provider links carry
 *     ?w=<id>&provider=<slug>, which now routes to Vox; bare ?w= (stale cached lib, blank
 *     CMS slug) stays on Yosi — the automatic fallback. Requires lib v1.19 + head snippet v4.
 * v1.0.1 (2026-07-14): TARGET moved to /p/33029 — the root 307 strips query params (verified live); deep path echoes all handoff params.
 *
 ...
 *
 * Never rewritten: bare provider ?w links WITHOUT a provider slug (Yosi both
 * arms — stale-cache fallback), embedded Yosi widgets (iframes),
 * /book-virtual-care-visit (until Vox confirms virtual in v1 — flip
 * VIRTUAL_ENABLED).
 */
(function () {
  'use strict';
  if (window.__acgVoxSplit) return;
  window.__acgVoxSplit = { version: '1.1.0', arm: null };
  ...
  function buildHandoff(srcParams) {
    ...
    // v1.1.0: Vox careType = service ?? visit_type (service wins); provider per Vox 8/8
    // confirm ("provider=provider-slug"). Slug-normalized like location.
    var svc = srcParams.get('service');
    if (svc) out.set('service', svc.toLowerCase().replace(/_/g, '-'));
    var prov = srcParams.get('provider');
    if (prov) out.set('provider', prov.toLowerCase().replace(/_/g, '-'));
    var attr = storedAttr();
  ...
  function bookingLink(a) {
    ...
    // v1.1.0: ?w= with a provider slug routes to Vox; bare ?w= (stale cached lib,
    // blank CMS slug) stays on Yosi — the automatic fallback.
    if (url.searchParams.get('w') && !url.searchParams.get('provider')) return null;
    return url;
  }
  ...
  function chokePoint() {
    ...
    // v1.1.0: bare ?w= (no provider slug) stays on Yosi; slug-carrying links redirect.
    if (pageParams.get('w') && !pageParams.get('provider')) return;
