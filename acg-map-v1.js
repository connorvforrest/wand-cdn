/*
  acg-map-v1.js (v1.0.0). The AllCare Georgia clinic map on the Google Maps JavaScript API.
  Replaces the Leaflet + CARTO map that ran on /locations and both landing pages.

  Loader line: <script src="https://cdn.allcare-ga.com/acg-map-v1.js"
  ...
  [data-acg-map] element. No such element and it does nothing. Clinic data comes from
  window.ACG (acg-personalization). See site-map/README.md for the deploy and page edits.
...
(function () {
  ...
  var mapEl = null, map = null, info = null, clinics = null;
  ...
  function cardFor(slug) { return document.querySelector('[data-clinic-slug="' + slug + '"]'); }
  ...
  function infoHtml(slug, loc, state) {
    ...
    return [
      ...
      '<span>&middot;</span><span>', st.hours, '</span>',
      dist ? '<span>&middot;</span><span>' + dist + '</span>' : '',
      '</div><div class="acg-pop-actions">',
      ...
      book ? ('<a class="acg-pop-btn is-primary" href="' + book + '" data-track="book_appointment"'
  ...
  function slugsInCardOrder() {
    var order = [];
    Array.prototype.forEach.call(document.querySelectorAll('[data-clinic-slug]'), function (c) {
  ...
  // Delegation on the list, so re-sorted CMS cards keep their wiring.
  function wireCards() {
    var root = document.querySelector('[data-clinic-list]') || document;
    root.addEventListener('mouseover', function (e) { setHover(e, true); });
    root.addEventListener('mouseout', function (e) { setHover(e, false); });
